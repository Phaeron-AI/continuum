"""CLI: train the world model.

Requires an existing token cache (built by build_token_cache) — training does
not build it, so the expensive artifact is created once and reused across
experiments. What training DOES do is verify the cache's tokenizer_version
matches what the model expects: a stale cache after a tokenizer retrain would
otherwise train the model on token ids that no longer mean what they did.

The model's vocab is read from the cache manifest, never configured by hand —
the head width must equal the tokenizer's vocab_size (stage 04), and reading
it makes that impossible to get wrong.

Drift evaluation is a SEPARATE command (evaluate_drift): autoregressive
rollout is expensive (64 sequential forward passes per frame) and is analysis,
not training. Keeping it out of the loop keeps training fast.
"""

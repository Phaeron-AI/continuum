...
def _psnr(a: Tensor, b: Tensor, data_range: float = 2.0) -> float:
  """PSNR between two [-1, 1] image tensors."""
...
def evaluate_drift(
  ...
  for f in range(num_frames):
    ...
    # the horizon extends only while quality holds, and stops at the first
    # frame that falls below threshold (drift is monotone in practice, but
    # we do not assume it)
